...
contract Verifierbisectfinal {
    
    uint32 constant n =   32768;
    uint16 constant nPublic =  14;
    uint16 constant nLagrange = 14;
    
    uint256 constant Qmx = 18538644148003687892590676593558270316975682211268094220490772100979371493069;
    uint256 constant Qmy = 4566590769041992558114846941296486773940556524938024014204359041267523118705;
    uint256 constant Qlx = 20214735100009600429786406745162956571760023548353458054558302958736792977006;
    uint256 constant Qly = 8224495027226627174225114239307866731859773429247398848961067312798044959785;
    uint256 constant Qrx = 10171927899870610043162172209747104843742296158847981304767535118728819028325;
    uint256 constant Qry = 21150697826575564617312666182541548765415527638867919831134277993328174331560;
    uint256 constant Qox = 15301341826026025718944160458543352191523950780084559985716295649210987054800;
    uint256 constant Qoy = 18938705904866655616668941016922026137783842217302109893740598887856943420299;
    uint256 constant Qcx = 4670243503257186532497608521088869109542973944653899959302245866550540230039;
    uint256 constant Qcy = 15797470784117459003909618178806448837832646957004189255700607181730246343414;
    uint256 constant S1x = 6232278714862965179720595905094201907494803309783322153589439326894989616349;
    uint256 constant S1y = 10946469057850283987661841017822060090079938565136682193490702439538927108922;
    uint256 constant S2x = 10511572617616933334953634122255268447036296006365256929659842792708302773801;
    uint256 constant S2y = 20888106547363935250503552881670229742962290704849134346010290517360454766789;
    uint256 constant S3x = 3931312379886315199797897653697099066416302685030616462819857750915314873435;
    uint256 constant S3y = 3347243366447072823545559176015648491055143910774149787570504615143128840841;
    ...
    function verifyProof(bytes memory proof, uint[] memory pubSignals) public view returns (bool) {
        assembly {
            ...
            function calculateChallanges(pProof, pMem, pPublic) {
                ...
                mstore( add(pMem, 1120 ), mload( add( pPublic, 32)))
                
                mstore( add(pMem, 1152 ), mload( add( pPublic, 64)))
                
                mstore( add(pMem, 1184 ), mload( add( pPublic, 96)))
                
                mstore( add(pMem, 1216 ), mload( add( pPublic, 128)))
                
                mstore( add(pMem, 1248 ), mload( add( pPublic, 160)))
                
                mstore( add(pMem, 1280 ), mload( add( pPublic, 192)))
                
                mstore( add(pMem, 1312 ), mload( add( pPublic, 224)))
                
                mstore( add(pMem, 1344 ), mload( add( pPublic, 256)))
                
                mstore( add(pMem, 1376 ), mload( add( pPublic, 288)))
                
                mstore( add(pMem, 1408 ), mload( add( pPublic, 320)))
                
                mstore( add(pMem, 1440 ), mload( add( pPublic, 352)))
                
                mstore( add(pMem, 1472 ), mload( add( pPublic, 384)))
                
                mstore( add(pMem, 1504 ), mload( add( pPublic, 416)))
                
                mstore( add(pMem, 1536 ), mload( add( pPublic, 448)))
                
                mstore( add(pMem, 1568 ), mload( add( pProof, pA)))
                mstore( add(pMem, 1600 ), mload( add( pProof, add(pA,32))))
                mstore( add(pMem, 1632 ), mload( add( pProof, add(pA,64))))
                mstore( add(pMem, 1664 ), mload( add( pProof, add(pA,96))))
                mstore( add(pMem, 1696 ), mload( add( pProof, add(pA,128))))
                mstore( add(pMem, 1728 ), mload( add( pProof, add(pA,160))))
                
                b := mod(keccak256(add(pMem, lastMem), 640), q) 
